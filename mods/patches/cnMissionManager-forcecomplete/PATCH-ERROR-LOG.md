...
| 2026-06-08 | Launcher `Invalid data file` on ACCESSING | Deployed UnityRaw bundle (~49 MB); FusionFall accepts **UnityWeb only** | **Forbidden:** `bundle-inject-raw`. See [`STRICT-RULES.md`](../../STRICT-RULES.md) §2.1 |
| 2026-06-08 | Bootstrap stall / no `Resources base url` with patched bundle | Any DLL change → `bundle-inject` LZMA recompress (~19 KB shrink); verify-il passes but runtime fails | Round-trip + shrink bisect before deploy; ERR-002 blocks ERR-001 |
| 2026-06-08 | offset-map / LZMA-props jar patch | No change to inject output hash (`1f5c728b…`) | Do not redeploy same path; need splice or client bundle |
| 2026-06-09 | doc504c deployed; user reports loading hang | **Process:** deployed without ACTIVE-ERROR-QUEUE / PATCH-ERROR-LOG update per STRICT-RULES §1. **Runtime:** log shows bootstrap OK (`Resources base url`, `CreateGameMode:2`) on `b41a56ee…`; hang is char-select / map-prefetch stall, not ERR-002 shrink. | Before any deploy: update cards §1 steps 2–4; record new hash bootstrap gate; do not conflate FE handshake stall (03:00) with DLL bootstrap |
| 2026-06-09 | doc504c `GetSelectedActiveMission` dnlib sync | **Root cause:** patched `GetSelectedActiveMission` breaks `SetMissionAndTaskFlags` after `CreateGameMode:5` — log shows task init but never `StartSelectMission` (compare 03:09 success on `40cb3fc2…` vs 03:20+ fail on `b41a56ee…`). | Never sync shared lookup methods for hotkey-only fallback; keep fallback inside FCT donor only if slot allows |
| 2026-06-09 | doc504d rollback deploy | Reverted `ApplyGetSelectedFallback` + dnlib `GetSelectedActiveMission` sync; stamp `doc504d`; bundle `f36fef9a…` 7117470 | Verify `StartSelectMission` + `CreateGameMode:24` after Connect |
