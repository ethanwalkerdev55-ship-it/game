...
|---------|------|----------|--------|-----------|--------|-----------|------------|
| 328 | GotoLocation | — | — | 329 | — | 726 | 908 |
| 329 | UseItems | — | — | 330 | — | — | 854 |
| 330 | Delivery | — | — | — | — | — | 726 |
...
- Prerequisite completed mission **81**.
...
Recover more Crazy Brain Candy from Galaxy Gardens.
...
- Complete at terminator NPC table ID **854** (proximity/talk).
...
**End packet** (`sP_CL2FE_REQ_PC_TASK_END`, opcode 318767116):
- `iTaskNum` = 329
- `iNPC_ID` = runtime ID from terminator NPC 854
- `iEscortNPC_ID` = escort runtime or **-1** if `bError=true`
- `iBox1Choice` / `iBox2Choice` = reward bits if reward UI shown
...
- Reward table ID **82** (journal reward UI before `TASK_END`).
