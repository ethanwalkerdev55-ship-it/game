# Mission 769 — Evil Takes Root 
(Part 2 of 3)

| Field | Value |
|-------|-------|
| Mission ID | 769 |
| Mission type | Normal |
| Task count | 2 |
| Has timer tasks | No |
| Has instance tasks | No |

## Task index

| Task ID | Type | Instance | Timers | Success → | Fail → | Grant NPC | Terminator |
|---------|------|----------|--------|-----------|--------|-----------|------------|
| 2689 | Defeat | — | — | 2690 | — | 3223 | — |
| 2690 | Talk | — | — | — | — | — | 3223 |

## Chain edges (from table)

- Success: **2689** → **2690**

## Task 2689

**Tags:** kill-quota, item-quota, chains-to=2690

### How this task is received

- Player accepts from NPC table ID **3223** via journal (`cnEvent(12,5)`).
- Prerequisite completed mission **768**.

### How this task must be completed

- Task type: **Defeat**
- Start at grant NPC table ID **3223** (journal accept or auto-chain).
- Complete: auto when quotas met, timer expires, or remote packet.
- Kill quotas: enemy 439 x0
- Collect items: item 564 x3

### Server packets

**Start packet** (`sP_CL2FE_REQ_PC_TASK_START`, opcode 318767115):
- `iTaskNum` = 2689
- `iNPC_ID` = runtime ID from grant NPC 3223
- `iEscortNPC_ID` = escort runtime ID if type 6

**End packet** (`sP_CL2FE_REQ_PC_TASK_END`, opcode 318767116):
- `iTaskNum` = 2689
- `iNPC_ID` = runtime ID from terminator NPC 0
- `iEscortNPC_ID` = escort runtime or **-1** if `bError=true`
- `iBox1Choice` / `iBox2Choice` = reward bits if reward UI shown

### Journal summary

Investigate the Ghastly Growths.

## Task 2690

**Tags:** standard

### How this task is received

- Started by auto-chain from prior task `ProcessEndSucc` → `RequestTaskStart`.
- Prerequisite completed mission **768**.

### How this task must be completed

- Task type: **Talk**
- Start: auto-chain or journal (no grant NPC table ID).
- Complete at terminator NPC table ID **3223** (proximity/talk).
- Reward table ID **681** (journal reward UI before `TASK_END`).

### Server packets

**Start packet** (`sP_CL2FE_REQ_PC_TASK_START`, opcode 318767115):
- `iTaskNum` = 2690
- `iNPC_ID` = runtime ID from grant NPC 0
- `iEscortNPC_ID` = escort runtime ID if type 6

**End packet** (`sP_CL2FE_REQ_PC_TASK_END`, opcode 318767116):
- `iTaskNum` = 2690
- `iNPC_ID` = runtime ID from terminator NPC 3223
- `iEscortNPC_ID` = escort runtime or **-1** if `bError=true`
- `iBox1Choice` / `iBox2Choice` = reward bits if reward UI shown

### Journal summary

Investigate the Ghastly Growths.
